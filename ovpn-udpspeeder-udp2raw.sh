...
systemctl stop myvpn 2>/dev/null

apt -y update
apt -y install curl openvpn
...
EasyRSA/easyrsa sign-req client myclient
...
${dir}/speeder.s -s -l0.0.0.0:6600 -r127.0.0.1:6601  -f20:10 -k "${udpspeederpwd}" --mode 0&
${dir}/udp2raw.s -s  -l0.0.0.0:6601 -r 127.0.0.1:6602 -k "${udp2rawpwd}" --raw-mode faketcp -a
EOF
...
cat <<EOF > ${dir}/stop.sh
#!/bin/bash
ps xf | grep -v grep | grep "speeder.s|udp2raw.s|openvpn" | while read pid _; do kill -9 "\$pid"; done
EOF
chmod +x ${dir}/stop.sh


rm -rf /etc/systemd/system/myvpn.service 2>/dev/null
cat <<EOF > /etc/systemd/system/myvpn.service
[Unit]
Description=MyVpn
...
EOF
chmod 664 /etc/systemd/system/myvpn.service
...
remote 127.0.0.1 7500  # Change this ip to your server ip.
...
echo .
echo This is your password, protect it.
echo udpspeeder: $udpspeederpwd udp2raw: $udp2rawpwd
echo The "ovpn.conf" is client config. You can download it to your local machine.
